'''This module collects some helper functions used in river.py.
'''
...
def readTXTFunctionFile(nameId, paraDict, paraName, textName):
    ...
    for line in lines:
        ...
        if name in paraDict:
            funName = re.split('\d+', name)[0]
            funName = funName + '0' + nameId + funID
            funID += 1
...
def fileParser(fname):
    ...
    for line in lines:
        ...
        if val.endswith('txt'):
            outdict = readTXTFunctionFile(outdict, name, val)
            nameID += 1
